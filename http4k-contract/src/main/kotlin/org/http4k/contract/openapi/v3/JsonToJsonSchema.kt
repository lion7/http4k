...
class JsonToJsonSchema<NODE>(
    ...
    override fun toSchema(obj: NODE, prefix: String?) = JsonSchema(obj, emptySet()).toSchema(prefix ?: "")

    private fun JsonSchema<NODE>.toSchema(prefix: String): JsonSchema<NODE> =
        when (json.typeOf(node)) {
            JsonType.Object -> objectSchema(prefix)
            JsonType.Array -> arraySchema(prefix)
    ...
    private fun JsonSchema<NODE>.arraySchema(prefix: String): JsonSchema<NODE> {
        val (node, definitions) = json.elements(node).toList().firstOrNull()?.let {
            JsonSchema(it, definitions).toSchema(prefix)
    ...
    private fun JsonSchema<NODE>.objectSchema(prefix: String): JsonSchema<NODE> {
        val (fields, subDefinitions) = json.fields(node)
            ...
            .fold(listOf<Pair<String, NODE>>() to definitions) { (memoFields, memoDefinitions), (first, second) ->
                JsonSchema(second, memoDefinitions).toSchema(prefix).let { memoFields + (first to it.node) to it.definitions }
            }
        ...
        val definitionId = prefix + "object" + newDefinition.hashCode()
